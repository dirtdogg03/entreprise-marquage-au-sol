...
const iconMap: Record<string, React.ReactNode> = {
  ...
  car: (
    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25m-3 0V5.25A2.25 2.25 0 0013.5 3h3.375c.621 0 1.125.504 1.125 1.125V9.75" />
    </svg>
  ),
  building: (
    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
      ...
      <div className="container-custom">
        ...
        <div className="mb-12">
          <motion.h3
            ...
            viewport={{ once: true }}
            className="text-xl font-black text-asphalt-800 mb-6 flex items-center gap-3"
          >
            <span className="w-10 h-10 bg-route-500 text-white flex items-center justify-center text-sm font-bold">
              {marquageServices.length}
            </span>
            Marquage au Sol
          </motion.h3>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {marquageServices.map((service, index) => (
              <ServiceCard key={service.id} service={service} location={location} index={index} />
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          >
            <span className="w-10 h-10 bg-route-500 text-white flex items-center justify-center text-sm font-bold">
              {signalisationServices.length}
            </span>
            Signalisation Verticale
          </motion.h3>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {signalisationServices.map((service, index) => (
              <ServiceCard key={service.id} service={service} location={location} index={index} />
            ))}
          </div>
        </div>
...
function ServiceCard({ service, location, index }: { service: Service; location: Location; index: number }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
    ...
    >
      <Link href={`/services/${service.slug}/${location.slug}`} className="group block h-full">
        <div className="card-signal h-full flex flex-col border-2 border-asphalt-100 hover:border-route-500">
          ...
          <div className="mt-5 flex-grow">
            <h4 className="text-lg font-black text-asphalt-800 group-hover:text-route-600 transition-colors">
              {service.name}
            </h4>
          ...
          {/* Arrow */}
          <div className="mt-4 flex items-center label-mono text-route-600">
            Voir ce service
